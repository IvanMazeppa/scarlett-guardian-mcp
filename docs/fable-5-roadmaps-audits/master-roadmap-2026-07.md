...
| 5.6 | **done** | `npc_canon` role prio 78 for secondary-characters-bible + family-dynamics; enum + retriever; registry tails on top NPCs (Shevchenko, Ryan, Lynn, Chris/Deb, Dan, Maya, Karin, Dr Berg) merged with agendas; reindex. Evidence: `wp-5.6-npc-canon-registry-2026-07-18.md`. |
...
| 5.7 | **done — defect open (WP-R2)** | `scene-roster.ts` cap 4; `presentCast` from `**Present:**`; preflight roster + soft exact-section expand; report.scene_roster; current-state Present line. Evidence: `wp-5.7-scene-roster-2026-07-18.md`. **Sol review 2026-07-20 confirmed:** short `sectionNeedle` values never resolve against full-ancestry manifest labels; expand fails silently every turn. Fix = WP-R2. |
...
| **R1** (un-deferred 3.5) | **done** | Duplex integrity: server floor `DEFAULT_DUPLEX_MIN_CHARS=200` + structure gate (`isSubstantialDuplexMessage`); POST `/duplex-cache` returns 422 on reject; bridge v2.1 smart scrape + last-good GM snapshot seed. Evidence: `wp-r1-duplex-integrity-evidence-2026-07-20.md`. Tests duplex-cache + full suite. |
| **R2** | pending | Exact-section contract repair: needle-aware `findSection` in RAG `manifest-index.ts` (unique suffix match, refuse ambiguity) + Guardian logs failed NPC expands to `retrieval_notes`. Real-manifest integration test. From Sol review (confirmed defect on WP-5.7) |
| **R3** | pending | Live/eval/backfill isolation: telemetry `source` tag + live-only summary default; eval uses `persist:false` serendipity + `bumpTurn:false` dramaturg (or temp rootDir). Acceptance: double `eval:fast` changes zero live `.guardian/` bytes; dashboard excludes eval events |
| **R4** | pending | Ensemble + negative-space goldens: 4-active+crowd, stealth ⚠ every applicable turn, dilution correction pair, Service-pressure contrastive cases (refusal valid, no invented operational detail). Gemini authors expectations |
