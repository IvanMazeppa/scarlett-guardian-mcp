...
| **ARCH (parking)** | backlog | Full Grok **raw thread export** before platform truncation — see `ideas-parking-raw-thread-export-2026-07-21.md` (Thread 7 loss). Not R1–R4. |
...
| 5.7 | **done** (R2 fixed expand contract) | `scene-roster.ts` cap 4; `presentCast` from `**Present:**`; preflight roster + soft exact-section expand; report.scene_roster; current-state Present line. Evidence: `wp-5.7-scene-roster-2026-07-18.md`. **Sol review 2026-07-20 confirmed:** short `sectionNeedle` values never resolve against full-ancestry manifest labels; expand fails silently every turn. Fix = WP-R2. |
